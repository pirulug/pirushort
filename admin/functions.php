...
function getUniqueHash(PDO $connect): string {
  $length   = LENGTH_HASH;
  $attempts = 0;  // Contador de intentos

  do {
    $short = getRandomString($length);
    $attempts++;
    logMessage("Intento $attempts: Hash generado '$short' con longitud $length");

    if (hashExists($short, $connect)) {
      logMessage("Colisión detectada! Aumentando longitud del hash...");
      $length++;  // Incrementar la longitud si el hash ya existe
    }
  } while (hashExists($short, $connect));

  logMessage("Hash único generado: '$short' con longitud final $length");
  return $short;
}

function hashExists(string $short, PDO $connect): bool {
  $query     = "SELECT 1 FROM links WHERE short = :short LIMIT 1";
  $statement = $connect->prepare($query);
  $statement->execute([':short' => $short]);
...
function getRandomString(int $length): string {
  ...
  for ($i = 0; $i < $length; $i++) {
    $randomString .= $source[random_int(0, $sourceLength - 1)];
  }
...
function logMessage(string $message): void {
  ...
  fwrite($fileHandle, date('Y-m-d H:i:s') . ' - ' . $message . PHP_EOL);
  fclose($fileHandle);
}